 ...
 */
var filter;
jQuery.expr[':'].Contains = function (a) {
    console.log(a);
    return (a.textContent || a.innerText || "").toUpperCase().indexOf(filter.toUpperCase()) >= 0;
...
function filterList(list) {
    $("#searchCustomer").change(function () {
        ...
        if (filter != '') {
            ...
            console.log($matches);
            $('.head-li', list).not($matches).slideUp();
            $matches.slideDown();
        } else {
            $(list).find(".head-li").slideDown();
        }
        return false;
    }).keyup(function () {
        $(this).change();
